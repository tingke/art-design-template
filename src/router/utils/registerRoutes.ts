    ...
    const checkRoutes = (routes: AppRouteRecord[], parentPath = '') => {
        routes.forEach((route) => {
            ...
            if (route.component) {
                ...
                if (componentPath && componentPath !== 'AppLayout') {
...
function loadComponent(componentPath: string, routeName: string): () => Promise<any> {
    ...
    // 检查是否是布局组件（RouterComponents 的 key）
    const layoutKeys: (keyof typeof RouterComponents)[] = ['AppLayout', 'Iframe']
    if (layoutKeys.includes(componentPath as keyof typeof RouterComponents)) {
        return RouterComponents[componentPath as keyof typeof RouterComponents]
    }
...
function convertRouteComponent(
    ...
    // 是否为一级菜单
    const isFirstLevel = depth === 0 && route.children?.length === 0 && component !== 'AppLayout'
...
function handleIframeRoute(
    ...
    const LAYOUT_VIEW = RouterComponents.AppLayout
    const IFRAME_VIEW = RouterComponents.Iframe
...
function handleLayoutRoute(
    ...
    converted.component = RouterComponents.AppLayout
